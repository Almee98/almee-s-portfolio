...
const Logo = () => {
    ...
    // the three elements, svg container, svg itself, and the solid image, are going to be hidden.
    // In order to manipulate them and apply gsap animations to them, we need to use useRef to
    // these 3 elements.

    // this reference is for the background, or "container"
    const bgRef = useRef()
    // this reference is for the outline of the logo
    const outlineLogoRef = useRef()
    // this reference is for the solid image
    const solidLogoRef = useRef()

    // in order to apply the animation effects, we will use useEffect.
    // for the array of the dependencies, set it as an empty array, as we want this to execute only once.
    useEffect(() => {
        // use gsap library for animation
        gsap.registerPlugin(DrawSVGPlugin)

        // the gsap.timeline() function will call the animations in a sequence.
        gsap.timeline()
        // the to() function will represent the end state of the animation.
        // 1st arg - element that we're going to animate.
        .to(bgRef.current, {
            duration: 1,
            opacity: 1,
        }) // this way, we created a fade-in animation for the outline.
